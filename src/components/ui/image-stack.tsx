        ...
        <div className="relative flex items-center justify-center w-96 h-96 my-12">
            {cards.map((card: Card, index: number) => {
                ...
                return (
                    <motion.div
                        key={card.id}
                        className="absolute w-64 origin-bottom-center overflow-hidden rounded-xl shadow-xl bg-neutral-900 cursor-grab active:cursor-grabbing border border-white/10"
                        style={{
                            zIndex: card.zIndex,
                            aspectRatio: '5/7'
                        }}
                    ...
                    >
                        <img
                            src={card.src}
                            alt={`Card ${card.id + 1}`}
                            className="absolute inset-0 h-full w-full object-cover rounded-lg pointer-events-none"
                            draggable={false}
                        />
                    </motion.div>
                );
