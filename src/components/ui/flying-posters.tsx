...
class Canvas {
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    if (Math.abs(this.start - y) > 6) this.moved = true;
  ...
  onTouchUp() {
    ...
    this.isDown = false;
    this.moved = false;
    // A press on the canvas that never travelled is a pick: open whichever
    // poster is sitting at the centre of the ring.
    if (wasDown && !wasDrag && this.onSelect && this.medias?.length) {
      const closest = this.medias.reduce((best, media) =>
        Math.abs(media.plane.position.y) < Math.abs(best.plane.position.y)
          ? media
          : best
      );
      this.onSelect(closest.index);
    }
  }
