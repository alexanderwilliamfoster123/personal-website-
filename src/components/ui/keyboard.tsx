...
// onKey reports every key press (virtual clicks AND physical keys are surfaced
// by the parent separately); `resolved` is the character honoring shift/caps.
export const Component = ({
  ...
  const [, setNumLock] = useState(true)
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      setPressedKey(e.key.length === 1 ? e.key.toLowerCase() : e.key)
  ...
  const handleKeyPress = (key: string, shiftSymbol?: string) => {
    setPressedKey(key)
    setTimeout(() => setPressedKey(null), 150)

    let resolved = key
    if (key.length === 1 && key.match(/[a-z]/)) {
      resolved = capsLock || shift ? key.toUpperCase() : key
    } else if (shift && shiftSymbol) {
      resolved = shiftSymbol
    }
    onKey?.(key, resolved)
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap justify-center gap-1">
              {numberRow.map((item) => (
                <KeyButton
                  key={item.key}
                  onClick={() => handleKeyPress(item.key, item.shiftSymbol)}
